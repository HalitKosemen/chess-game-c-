...
int piyon_olasi_hareketler(int piyon_index, int hamle_index) {
	...
	std::vector<int> dizi_1 = siyah_piyon_konumlari();
	std::vector<int> dizi_toplam = dizi;
	dizi_toplam.insert(dizi_toplam.end(), dizi_1.begin(), dizi_1.end());

	int fark = hamle_index - piyon_index;
	for (size_t i = 0;i < dizi_toplam.size();i++) {
		char tas_harf = (char)santrac_tahtasi[dizi_toplam[i]];
		int renk_kontrol = (bool)tas_rengi[dizi_toplam[i]];
		if (renk_kontrol == 1 && tas_harf == 'P') {
