...
std::vector<int> piyonlarin_olasi_hamlelerini_bulma_fonks() { 
	...
	std::vector<int> butun_piyon_olasi_hamleler;
	for (int i = 0;i < 64;i++) {
	...
	for (size_t i = 0; i < butun_beyaz_piyon_konum_indx.size();i++) {
		...
		for (int j = 0;j < 64;j++) {
			oynancak_hamle_konum_index = j;
			if (piyon_hareket_okey_mi()) {
				butun_piyon_olasi_hamleler.push_back(j);
			}
		}
	}

	cout << "butun olasi piyon hamleleri :";
	for (size_t j = 0;j < butun_piyon_olasi_hamleler.size();++j) {
		cout << butun_piyon_olasi_hamleler[j] << "";
	}
	cout << endl;
	...
	return butun_piyon_olasi_hamleler;
}

std::vector<int> kalelerin_olasi_hamleleri_bulma_fonks() {
	...
	std::vector<int> butun_kale_olasi_hamleler;
	for (int i = 0;i < 64;i++) {
	...
	for (size_t i = 0;i < butun_beyaz_kale_konum_indx.size();i++) {
		oynancak_tas_konum_index = butun_beyaz_kale_konum_indx[i];
		for (int j = 0;j < 64;j++) {
			oynancak_hamle_konum_index = j;
			if (kale_hareket_okey_mi()) {
				butun_kale_olasi_hamleler.push_back(j);
			}
		}
	}

	cout << "butun olasi kale hamleleri :";
	for (size_t j = 0;j < butun_kale_olasi_hamleler.size();++j) {
		cout << butun_kale_olasi_hamleler[j] << "";
	}
	cout << endl;
	...
	return butun_kale_olasi_hamleler;
}
...
int main() {
	...
	while (game_continous) {
		...
		if (kullanicinin_girdigi_hamleler_okey_mi(oynancak_tas_konumu, oynancak_hamle_konumu)) {
			piyonlarin_olasi_hamlelerini_bulma_fonks();
			kalelerin_olasi_hamleleri_bulma_fonks();
			if (piyon_hareket_okey_mi()) {
