...
int girilen_hamlenin_index_hesaplamasi(std::string& girilen_deger) {
	...
	char birinci_eleman = girilen_deger[0]; //satýr sutun harf ve sayý atamasý yapýlabiliyor
	char ikinci_eleman = girilen_deger[1];
	if (birinci_eleman < 'a' || birinci_eleman > 'h') {
		return -1;
	}
	if (ikinci_eleman < '1' || ikinci_eleman >'8') {
		return -1;
	}
	int birinci_eleman_indx = birinci_eleman - 'a';
	int ikinci_eleman_indx = ikinci_eleman - '1';
...
int main() {
	...
	while (game_continous) {
		...
		int oynancak_tas_konum_index = girilen_hamlenin_index_hesaplamasi(oynancak_tas_konumu);
		cout << "sectiginiz tas gitmesini istediginiz konum :";
		cin >> oynancak_hamle_konumu;
		int oynancak_hamle_konum_index = girilen_hamlenin_index_hesaplamasi(oynancak_hamle_konumu);
		piyon_hareket_kontrol(oynancak_tas_konum_index, oynancak_hamle_konum_index,true);
