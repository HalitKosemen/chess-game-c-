...
//deneme_11 kodu gayet iyi çalýþýyor fakat deneme_12 de eksiklikler var ...
...
std::vector<int> olasi_hamleler_1(int olasi_fonks_hamle_indx) { //oynancak tasin hamle indeksi gelicek
	...
	cout << "oynanan_tas_olasi_hamleleri :";
...
std::vector<int> olasi_beyaz_piyon_hamleleri(int olasi_fonks_hamle_indx) {

	std::vector<int> beyaz_piyon_hamle_olasiliklari;
	int renk_kontrol = (bool)tahta_kutu_renk_kontrol(oynancak_tas_konumu);
	char tas_harf = (char)tahta_kutu_tas_kontrol(oynancak_tas_konumu);
	...
	if (renk_kontrol == 1 ) {
		for (int i = 0;i < 64;i++) {
			oynancak_hamle_konum_index = hamle_index;
			if (piyon_hareket_okey_mi()) {
				beyaz_piyon_hamle_olasiliklari.push_back(hamle_index);
			}
			hamle_index++;
		}
		oynancak_hamle_konum_index = oynn_hamle_index;
	}
	cout << "butun_beyaz_tas_hamleleri:";
	for (size_t j = 0;j < beyaz_piyon_hamle_olasiliklari.size();++j) {
		cout << beyaz_piyon_hamle_olasiliklari[j] << "";
	}
	cout << endl;
	return beyaz_piyon_hamle_olasiliklari;
	
}

std::vector<int> olasi_butun_beyaz_tas_hamleleri() {

	std::vector<int> butun_beyaz_olasi_hamleler;
	int renk_kontrol = (bool)tahta_kutu_renk_kontrol(oynancak_tas_konumu);
	char tas_harf = (char)tahta_kutu_tas_kontrol(oynancak_tas_konumu);
	...
	if (renk_kontrol == 1) {
		for (int i = 0;i < 64;i++) {
			oynancak_hamle_konum_index = hamle_index;
			if (piyon_hareket_okey_mi()) {
				butun_beyaz_olasi_hamleler.push_back(hamle_index);
			}
			if (kale_hareket_okey_mi()) {
				butun_beyaz_olasi_hamleler.push_back(hamle_index);
			}
	...
	cout << "butun_beyaz_tas_hamleleri:";
	for (size_t j = 0;j < butun_beyaz_olasi_hamleler.size();++j) {
		cout << butun_beyaz_olasi_hamleler[j] << "";
	}
	cout << endl;
	return butun_beyaz_olasi_hamleler;
...
int main() {
	...
	while (game_continous) {
		...
		if (kullanicinin_girdigi_hamleler_okey_mi(oynancak_tas_konumu, oynancak_hamle_konumu)) {
			olasi_hamleler_1(oynancak_tas_konum_index);
			olasi_butun_beyaz_tas_hamleleri();
			if (piyon_hareket_okey_mi()) {
