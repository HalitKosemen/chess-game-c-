...
int tahta_kutu_renk_kontrol(std::string girilen_hamle) {
	int index_hesap = girilen_hamlenin_index_hesaplamasi(girilen_hamle);
	bool tas_rengi_ne = santrac_tahtasi[index_hesap];
	return tas_rengi_ne;
}
...
int main() {
	...
	while (game_continous) {
		...
		piyon_hareket_kontrol(oynancak_tas_konum_index, oynancak_hamle_konum_index, false);
		...
		tas_rengi[oynancak_tas_konum_index] = false;
		tahta_yazdirma();
