...
//siyah piyon olasý hamleleri buna dahil deðil çünkü siyah piyon ileri oynayabiliyor bu kareyi sahýn oynayabilceði hamlelerden magrum býrakmamamýz lazým bu yüzden piyon olasý hamleleri kaldýrdýr.
std::vector<int> siyah_butun_olasi_hamleler(std::vector<int> v1, std::vector<int> v2, std::vector<int> v3, std::vector<int> v4 ) {
	std::vector<int> sonuc;
	sonuc.reserve(v1.size() + v2.size() + v3.size() + v4.size());
	for (int eleman : v1) {
...
bool beyaz_sah_tehdit_altinda_mi_2() {
	...
	for (const auto& konum : vezir_konumlari) {
		if (s_vezir_olasi_hareketler(konum, beyaz_sah_konum_index)) {
			return true;
		}
	}
	for (const auto& konum : fil_konumlari) {
		if (s_fil_olasi_hareketler(konum, beyaz_sah_konum_index)) {
			return true;
		}
	}
	for (const auto& konum : at_konumlari) {
		if (s_at_olasi_hareketler(konum, beyaz_sah_konum_index)) {
			return true;
		}
	}
	for (const auto& konum : kale_konumlari) {
		if (s_kale_olasi_hareketler(konum, beyaz_sah_konum_index)) {
			return true;
		}
	}
	for (const auto& konum : piyon_konumlari) {
		if (s_piyon_olasi_hareketler(konum, beyaz_sah_konum_index)) {
			return true;
		}
	}
	return false;
...
int sah_olasi_hareketler(int hamle_index) {
	std::vector<int> sonuc = siyah_butun_olasi_hamleler(olasi_hamleler_10(), olasi_hamleler_12(), olasi_hamleler_13(), olasi_hamleler_14());
	std::vector<int> piyon_konumlari = siyah_piyon_konumlari();
	int sah_konum = 0;
	for (int i = 0;i < 64;i++) {
		if (santrac_tahtasi[i] == 'S' && tas_rengi[i] == true) {
	...
	int sah_satir = (sah_konum / 8) + 1;
	int sah_sutun = (sah_konum % 8) + 1;
	int hamle_satir = (hamle_index / 8) + 1;
	int hamle_sutun = (hamle_index % 8) + 1;

	//satir_farki = 1 sutun_farki = 0 , satir_farki = 0 sutun_farki = 1 , satir_farki = 1 sutun_farki = 1 olabilcek durumlar
	int satir_farki = abs(hamle_satir - sah_satir);
	int sutun_farki = abs(hamle_sutun - sah_sutun);

	if ((satir_farki == 1 && sutun_farki == 0) || (satir_farki == 0 && sutun_farki == 1) || (satir_farki == 1 && sutun_farki == 1)) {
		for (size_t k = 0;k < sonuc.size();k++) {
			if (hamle_index == sonuc[k]) {
				return -1;
			}
			int piyon_satir;
			int piyon_sutun;
			int satir_farki_1;
			int sutun_farki_1;
			for (size_t l = 0;l < piyon_konumlari.size();l++) {
				piyon_satir = (piyon_konumlari[l] / 8) + 1;
				piyon_sutun = (piyon_konumlari[l] % 8) + 1;
				satir_farki_1 = (hamle_satir - piyon_satir); //eðer bu deðerler - ise hamle oynanamaz
				sutun_farki_1 = (hamle_sutun - piyon_sutun);
			}
			if ((hamle_index == sonuc[k]) && (satir_farki_1 < 0 && sutun_farki_1 < 0)) {
				return -1;
			}
			if ((hamle_index != sonuc[k]) && (satir_farki_1 > 0 && sutun_farki_1 > 0)) {
				return hamle_index;
...
std::vector<int> sah_olasi_hamleler() {
	std::vector<int> hamleler_dizisi_sah;
	int sonuc;
	for (int j = 0;j < 64;j++) {
		sonuc = sah_olasi_hareketler(j);
		if (sonuc != -1) {
			hamleler_dizisi_sah.push_back(j);
		}
	}
	cout << "olasi sah hamleleri :";
	for (size_t j = 0;j < hamleler_dizisi_sah.size();++j) {
		cout << hamleler_dizisi_sah[j] << " ";
	}
	cout << endl;
	return hamleler_dizisi_sah;
}
...
int main() {
	...
	while (game_continous) {
		...
		if (kullanicinin_girdigi_hamleler_okey_mi(oynancak_tas_konumu, oynancak_hamle_konumu)) {
			...
			if (beyaz_sah_tehdit_altinda_mi_2()) {
				cout << "sah_cekildi";
			}
		}
	}
